...
export class ReportsComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  constructor(
    public skills: SkillControllerService,
    private ref: ChangeDetectorRef,
    private settingService: SettingControllerService,
    private batchService: BatchControllerService,
    private curriculaService: CurriculumControllerService,
    private userInfoService: AuthService,
    private trainerService: TrainerControllerService /*private notificationService: NotificationService */
  ) {
  ...
  dataSource = new MatTableDataSource(this.skillList);
  dataSource1 = new MatTableDataSource(this.skillList);
  ...
  //
  //       }},
  //     series: this.skills.retrieveAllSkills()
  //     ,
  //   }
  // );
  // Trainer = new Chart({
  ...
  ngOnInit() {
    this.isAdmin = false;
    if (this.userInfoService.userHasRole(['VP of Technology'])) {
      this.isAdmin = true;
    }
  ...
  calcStartDate(requiredDate, index, evt) {
    ...
    const formattedDate =
      this.monthList[startDate.getMonth()] +
      '-' +
      startDate.getDate() +
      '-' +
